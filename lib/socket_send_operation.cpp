	...
	cppcoro::detail::async_operation_base& operation) noexcept
{
	...
	if (result == SOCKET_ERROR)
	{
		...
		if (errorCode != WSA_IO_PENDING)
		{
			...
			operation.m_ioState.m_numberOfBytesTransferred = numberOfBytesSent;
			return false;
		}
	}
	...
	cppcoro::detail::async_operation_base& operation) noexcept
{
	...
	operation.m_ioState.m_completeFunc = [&]() {
		return send(m_socket.native_handle(), m_buffer, m_byteCount, 0);
	};
	operation.m_ioState.m_ioService->get_io_context().watch_handle(m_socket.native_handle(), reinterpret_cast<void*>(&operation), detail::watch_type::writable);
	return true;
}
#endif
