...
{
	operation.m_ioState.m_handle = m_fileHandle;
	const DWORD numberOfBytesToWrite =
		m_byteCount <= 0xFFFFFFFF ?
		static_cast<DWORD>(m_byteCount) : DWORD(0xFFFFFFFF);

	operation.m_ioState.get_overlapped()->Offset = static_cast<cppcoro::detail::win32::dword_t>(m_offset);
	operation.m_ioState.get_overlapped()->OffsetHigh = static_cast<cppcoro::detail::win32::dword_t>(m_offset >> 32);
	...
	if (!ok && errorCode != ERROR_IO_PENDING)
	{
		...
		operation.m_ioState.m_errorCode = errorCode;
		operation.m_ioState.m_numberOfBytesTransferred = numberOfBytesWritten;
...
bool cppcoro::file_write_operation_impl::try_start(
	cppcoro::detail::async_operation_base& operation) noexcept
{
	operation.m_ioState.m_fd = m_fileHandle;
	auto seek_res = lseek(m_fileHandle, m_offset, SEEK_SET);
	if (seek_res < 0) {
		operation.m_ioState.m_res = -errno;
		return false;
	}
	operation.m_ioState.m_completeFunc = [&]() {
		return write(m_fileHandle, m_buffer, m_byteCount);
	};
	operation.m_ioState.m_ioService->get_io_context().watch_handle(m_fileHandle, reinterpret_cast<void*>(&operation), detail::watch_type::writable);
	return true;
}
#endif
