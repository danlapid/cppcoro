	...
	cppcoro::detail::async_operation_base& operation) noexcept
{
	...
	if (!ok && errorCode != ERROR_IO_PENDING)
	{
		...
		operation.m_ioState.m_numberOfBytesTransferred = numberOfBytesRead;
	...
	cppcoro::detail::async_operation_base& operation) noexcept
{
	...
	operation.m_ioState.m_ioService->get_io_context().watch_handle(m_fileHandle, reinterpret_cast<void*>(&operation), detail::watch_type::readable);
	return true;
}
#endif
